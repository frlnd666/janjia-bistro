    ...
    <div style={{ minHeight: '100dvh', background: 'var(--bg)', display: 'flex', flexDirection: 'column', position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '48px 24px 32px', position: 'relative', zIndex: 1 }}>
        ...
        <div style={{ width: '100%', maxWidth: '320px', display: 'flex', flexDirection: 'column', gap: '12px', marginBottom: '32px' }}>
          <Link href={tableId ? `/menu?table=${tableId}` : '/menu'} style={{
            display: 'flex', alignItems: 'center', justifyContent: 'space-between',
            background: 'var(--accent)', borderRadius: '18px', padding: '18px 22px',
            textDecoration: 'none', transition: 'transform 0.15s, box-shadow 0.15s',
            boxShadow: '0 8px 32px rgba(232,114,58,0.4)',
          }}>
          ...
          <Link href="/scan" style={{
            display: 'flex', alignItems: 'center', justifyContent: 'space-between',
            background: 'var(--surface-2)', borderRadius: '18px', padding: '18px 22px',
            border: '1px solid var(--border-strong)', textDecoration: 'none', transition: 'border-color 0.15s',
          }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
        ...
        <div style={{ width: '100%', maxWidth: '320px', background: 'var(--surface-1)', borderRadius: '20px', border: '1px solid var(--border)', padding: '20px', marginBottom: '24px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
            {[
              { n:'01', icon: QrCode, text: 'Scan QR di meja kamu' },
              { n:'02', icon: UtensilsCrossed, text: 'Pilih menu favoritmu' },
              { n:'03', icon: Clock, text: 'Tunggu pesanan datang' },
              { n:'04', icon: Star, text: 'Bayar di kasir saat selesai' },
            ].map(({ n, icon: Icon, text }) => (
              <div key={n} style={{ display: 'flex', alignItems: 'center', gap: '14px' }}>
                <span style={{ fontSize: '11px', fontWeight: 700, color: 'var(--text-muted)', minWidth: '20px', fontVariantNumeric: 'tabular-nums' }}>{n}</span>
      ...
      <div style={{ textAlign: 'center', padding: '16px', position: 'relative', zIndex: 1 }}>
        ...
        <Link href="/login" style={{ fontSize: '11px', color: 'var(--text-muted)', marginTop: '4px', display: 'block', textDecoration: 'none' }}>Staff Login</Link>
      </div>
    </div>
  )
}
